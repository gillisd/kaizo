...
[Spec description prose](#spec-description-prose)), **`Kaizo/ExplicitBegin`**,
which requires an explicit `begin` for method bodies that `rescue` or `ensure`
(see [Explicit begin](#explicit-begin)), and **`Kaizo/NextInNonVoidEnumerable`**,
