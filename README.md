...
[Comments in specs](#comments-in-specs)), **`Design/ExplicitBegin`**, which
requires an explicit `begin` for method bodies that `rescue` or `ensure` (see
[Explicit begin](#explicit-begin)), and **`Design/NextInNonVoidEnumerable`**,
...
`Design/ExplicitBegin` requires an explicit `begin`/`end` block when a method
...
Style/RedundantBegin:
  Enabled: true     # opt back out of Design/ExplicitBegin
```

## Next in value-returning blocks

`Design/NextInNonVoidEnumerable` flags `next` inside the block of a
...
```yaml
Design/NextInNonVoidEnumerable:
  AllowedMethods:
