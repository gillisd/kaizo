...
**Applies to:** `Design/PositionalArguments`, `Design/KeywordArguments`,
`Design/TotalArguments`, and their shared `ArgumentCounting` module
([`lib/rubocop/cop/design/argument_counting.rb`](../lib/rubocop/cop/design/argument_counting.rb)).
...
**Short answer:** the three `Design/*` argument cops are not a reimplementation
...
### The `Design/*` argument cops (this gem)

Source: [`argument_counting.rb`](../lib/rubocop/cop/design/argument_counting.rb)
...
- **Three cops, three independent `Max` options.** `positional_arity` counts
  ...
  ([`argument_counting.rb:33-44`](../lib/rubocop/cop/design/argument_counting.rb)).
- **Only four parameter types are ever counted:** `arg`, `optarg`, `kwarg`,
  ...
  ([`POSITIONAL_TYPES`/`KEYWORD_TYPES`, lines 12-13](../lib/rubocop/cop/design/argument_counting.rb)).
- **Only method *definitions* are inspected:** `def`, `def self.`,
  `define_method`, and `define_singleton_method` — nothing else
  ([`on_def`/`on_defs` and the `DEFINE_METHODS`-gated `on_block`, lines 18-29](../lib/rubocop/cop/design/argument_counting.rb)).
...
- Department `Design` — *domain-modeling pressure*, framed by the README around
...
positional and keyword arity *separately at the same time*. `Design/KeywordArguments`
...
```
rubocop-design  Design/KeywordArguments Max 2:
...
Even `Design/TotalArguments` is **not** equivalent to `Metrics/ParameterLists`
...
def m(a, b, *rest, **opts)          (2 named + splat + double-splat)
   Design/TotalArguments (Max 2)  : no offense           # counts a, b = 2
...
excluded ([`total_arguments.rb:11-13`](../lib/rubocop/cop/design/total_arguments.rb),
and the `does not count splat, double-splat, or block arguments` spec in
[`total_arguments_spec.rb`](../spec/rubocop/cop/design/total_arguments_spec.rb)).
...
| | `Metrics/ParameterLists` | `Design/*Arguments` |
|---|---|---|
| Department | `Metrics` (complexity budget) | `Design` (domain-modeling pressure) |
...
turning the cop off. A `Metrics` cop says *"reduce complexity."* A `Design` cop,
...
([`allowed_initialize?`, `argument_counting.rb:54-70`](../lib/rubocop/cop/design/argument_counting.rb);
...
`Design`-department framing — are things core neither does nor can be configured
...
p run(RuboCop::Cop::Design::TotalArguments,       { "Max" => 2 }, src)                    # => []
